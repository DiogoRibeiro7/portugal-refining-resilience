...
$0.73$ to $1.01$ after the transition ($p<0.001$); the equivalent gasoline coefficient does
not move ($p=0.80$). The association between reduced domestic diesel manufacturing and
increased exposure to Spanish diesel prices is consistent across the physical and price
evidence, but 2022 confounds the post-closure window and this report does not claim a causal
effect of the closure itself.
\end{abstract}
...
the pair is cointegrated ($p=0.000026$), so an error-correction model is indicated; the
short-run model is reported for comparability and the ECM is left to further work.
...
estimated. The gasoline interaction is effectively zero and statistically indistinguishable
from no change. The contrast is informative: diesel is the product whose domestic manufacturing
fell, and diesel is the product whose price now tracks Spain almost one-for-one, while gasoline,
which remained in domestic surplus, shows no change in pass-through.
...
approximately $1.01$ across the transition, while gasoline pass-through is unchanged. The
physical and price evidence point the same way for diesel, and neither points that way for
gasoline, which is what a capability-driven mechanism would predict.
